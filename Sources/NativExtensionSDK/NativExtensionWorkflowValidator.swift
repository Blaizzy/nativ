...
public enum NativExtensionWorkflowValidator {
    ...
    public static let implementedOperations: Set<NativWorkflowOperation> = [
        .readSelection, .invokeModel, .replaceSelection, .readStorage, .writeStorage,
    ]
    ...
    public static func validate(
        ...
        var outputs: [String: [String: NativWorkspaceValueType]] = [:]
        ...
        for step in workflow.steps {
            ...
            case .readStorage, .writeStorage: input = "key"
            default: throw NativExtensionWorkflowError.unimplementedOperation(step: step.id, operation: step.type)
            }
            let expectedInputs = operation == .writeStorage ? Set(["key", "value"]) : Set(input.map { [$0] } ?? [])
            ...
            var storageType: NativWorkspaceValueType?
            if operation == .readStorage || operation == .writeStorage {
                guard manifest.schemaVersion == 2,
                      let key = step.inputs["key"]?.text,
                      NativExtensionDashboardValidator.validKey(key),
                      let field = dashboard?.storage[key] else {
                    throw invalid(step, "Storage keys must be literal fields declared in Dashboard.json.")
                }
                storageType = field.type
                if operation == .writeStorage, let value = step.inputs["value"] {
                    if case .text(let template) = value, NativWorkflowReference.hasMalformedReferences(in: template) {
                        throw invalid(step, "Malformed storage value binding.")
                    }
                    if case .text(let template) = value, !value.references.isEmpty {
                        let references = value.references
                        let reference = references[0]
                        let token = reference.output.map { "{{\(reference.step).\($0)}}" } ?? "{{\(reference.step)}}"
                        if template == token {
                            guard outputs[reference.step]?[reference.output ?? "text"] == field.type else {
                                throw invalid(step, "The referenced value does not match the storage field type.")
                            }
                        } else if field.type != .text || template.utf8.count > maximumTextBytes {
                            throw invalid(step, "Only text fields support interpolated templates.")
                        }
                    } else if !field.type.accepts(value) {
                        throw invalid(step, "The value does not match the storage field type.")
                    }
                }
            }
            ...
            switch operation {
            case .replaceSelection, .writeStorage: outputs[step.id] = [:]
            case .readStorage:
                if let storageType { outputs[step.id] = ["value": storageType] }
            default: outputs[step.id] = ["text": .text]
            }
        }
        for trigger in workflow.triggers {
            guard let selected = trigger.steps else { continue }
            guard manifest.schemaVersion == 2, !selected.isEmpty,
                  Set(selected).count == selected.count,
                  selected.allSatisfy({ outputs[$0] != nil }) else {
                throw NativExtensionWorkflowError.invalidWorkflow("Command steps must be unique existing IDs in a schema 2 package.")
            }
            var available: [String: [String: NativWorkspaceValueType]] = [:]
            var selectionAvailable = false
            for step in workflow.steps(forCommand: trigger.commandID ?? "") {
                try verifyReferences(in: step, outputs: available)
                if step.operation == .replaceSelection, !selectionAvailable {
                    throw invalid(step, "This command must read a selection before replacing it.")
                }
                selectionAvailable = selectionAvailable || step.operation == .readSelection
                available[step.id] = outputs[step.id]
            }
        }
    }
    ...
    private static func verifyReferences(
        ...
        outputs: [String: [String: NativWorkspaceValueType]]
    ) throws {
        for value in step.inputs.values {
            for reference in value.references {
                ...
                guard produced[output] != nil else {
