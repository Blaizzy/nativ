...
public struct NativExtensionManifest: Codable, Hashable, Identifiable, Sendable {
    ...
    /// Relative name of the workflow document. Optional for dashboard-only declarative
    /// workspaces, meaningless to the other runtimes.
...
public enum NativExtensionManifestError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .missingWorkflow:
            "Declare Workflow.json, or a schema 2 dashboard with no commands."
        case .unexpectedRuntimeBundleIdentifier:
...
public enum NativExtensionManifestValidator {
    public static func validate(
        ...
        guard (1...NativExtensionManifest.currentSchemaVersion).contains(manifest.schemaVersion) else {
        ...
        case .declarative:
            guard manifest.workflow == NativExtensionManifest.workflowDocumentName
                || (manifest.schemaVersion == 2 && manifest.workflow == nil && manifest.dashboard == NativExtensionManifest.dashboardDocumentName && manifest.contributions.commands.isEmpty) else {
