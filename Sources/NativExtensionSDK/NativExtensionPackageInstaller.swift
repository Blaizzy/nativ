...
public enum NativExtensionPackageError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .unsupportedDashboard:
            "A dashboard requires manifest schema 2, Dashboard.json, and exactly one sidebar contribution."
        case .documentTooLarge(let name):
            "\(name) exceeds the 1 MiB document limit."
        case .missingDashboardDocument:
            "The extension package does not contain Dashboard.json."
        case .missingWorkflowDocument:
...
public struct NativExtensionInstalledPackage: Sendable {
    ...
    public let dashboard: NativExtensionDashboard?
...
public struct NativExtensionPackageInstaller {
    ...
    private func readDocument(at url: URL) throws -> Data {
        ...
        guard properties.isRegularFile == true, properties.isSymbolicLink != true else {
            throw NativExtensionPackageError.malformedManifest("\(url.lastPathComponent) must be a regular file, not a link.")
        }
    ...
    public func loadWorkflow(
        ...
        let dashboard = try loadDashboard(at: packageURL, manifest: manifest)
        guard manifest.workflow != nil else { return nil }
        ...
        try NativExtensionWorkflowValidator.validate(workflow, manifest: manifest, dashboard: dashboard)
        return workflow
    }
    ...
    public func loadDashboard(
        at packageURL: URL,
        manifest: NativExtensionManifest
    ) throws -> NativExtensionDashboard? {
        guard manifest.runtime == .declarative else { return nil }
        guard manifest.dashboard != nil || !manifest.contributions.sidebar.isEmpty else { return nil }
        guard manifest.schemaVersion == 2,
              manifest.dashboard == NativExtensionManifest.dashboardDocumentName,
              manifest.contributions.sidebar.count == 1 else {
            throw NativExtensionPackageError.unsupportedDashboard
        }
        let url = packageURL.appendingPathComponent(NativExtensionManifest.dashboardDocumentName)
        guard fileManager.fileExists(atPath: url.path) else {
            throw NativExtensionPackageError.missingDashboardDocument
        }
        let dashboard: NativExtensionDashboard
        do {
            dashboard = try JSONDecoder().decode(NativExtensionDashboard.self, from: readDocument(at: url))
        } catch let error as DecodingError {
            throw NativExtensionPackageError.malformedManifest(Self.describe(error, document: "Dashboard.json"))
        }
        try NativExtensionDashboardValidator.validate(dashboard, manifest: manifest)
        return dashboard
    }
        ...
        for entry in entries {
            ...
            do {
                ...
                manifests[manifest.id] = NativExtensionInstalledPackage(
                    ...
                    dashboard: try loadDashboard(at: entry, manifest: manifest)
                )
            } catch {
                issues.append(
