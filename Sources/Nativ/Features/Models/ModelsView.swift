...
private struct HubModelRow: View, @MainActor Equatable {
    ...
    private var resolvedMemoryEstimate: LocalModelMemoryEstimate? {
        guard let downloadSizeBytes, downloadSizeBytes > 0 else {
            return model.memoryEstimate
        }
        return LocalModelMemoryEstimate(
            modelBytes: Double(downloadSizeBytes),
            activationReserveBytes: LocalModelMemoryEstimate.activationReserveBytes(
    ...
    private var memoryFitWarning: HubModelMemoryFitWarning? {
        guard let estimate = resolvedMemoryEstimate, !estimate.isUsable else {
            return nil
        }
