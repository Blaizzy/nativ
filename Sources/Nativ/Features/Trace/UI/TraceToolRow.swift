...
/// A collapsed burst of routine tool work.
struct TraceToolRunRow: View {
    let run: TraceToolRun
            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 6) {
                    ForEach(run.items) { item in
                        if case .tool(let tool) = item.body {
                            TraceToolRow(tool: tool)
                        }
                    }
                }
